#!/usr/bin/env bash
# Build standalone PDF for "Theoretical Foundations of Transmutative Machine Systems"
# (English version of Theoretische Grundlagen).
# Requires: pandoc, pdflatex (e.g. from TeX Live).
# Run from repo root: ./scripts/build-pdf-theoretische-en.sh

set -e
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
INPUT="$REPO_ROOT/markdown/Theoretische-Grundlagen.md"
DIST="$REPO_ROOT/dist"
PDF="$DIST/theoretische-grundlagen-en.pdf"
