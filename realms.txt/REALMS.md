...
```mermaid
flowchart LR
  subgraph obs [Observer realm]
    Decoherence[Observation / decoherence]
    Planck[Resolution limit: Planck]
    Room[Observer room]
  end
  Decoherence --> Planck --> Room
  Objective[Objective state] -.->|constrains| Room
  Analog[Analog / unknown computation] -.->|modifies interpretation| Room
```
